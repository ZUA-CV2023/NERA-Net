...
def get_triple_loss(anchors, local_features, mask, class_chosen):
    ...
    """
    b, c, h, w = local_features.shape   # [bs, 256, 1, 1]
    mask = F.interpolate(mask.float(), size=(h, w), mode="nearest").long().view(b, -1)      # [bs, 225]
    local_features = local_features.view(b, c, -1).permute(0, 2, 1).contiguous()    # [bs, 255, 256]
    ...
    for i in range(b):
        ...
        anchor_list_i_mu = anchors[i][int(class_chosen[0][0])].unsqueeze(0)     # [1, 256]
        for sample_i in range(length):
            anchor_list.append(anchor_list_i_mu)
        anchor_list = torch.cat(anchor_list, dim=0)    # [1, 256]
...
def hard_triplet_dig(anchor, positive, negative):
    ...
    for i in range(anchor.shape[0]):
        edu_distance_pos = F.pairwise_distance(F.normalize(anchor[i], p=2, dim=-1),
                                               F.normalize(positive, p=2, dim=-1))      # [52]
        edu_distance_neg = F.pairwise_distance(F.normalize(anchor[i], p=2, dim=-1),     # [1]
                                               torch.mean(F.normalize(negative, p=2, dim=-1), dim=0, keepdim=True))
        neg_val, _ = edu_distance_neg.sort()    # [1]
        pos_val, _ = edu_distance_pos.sort()    # [52]
...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        reduce_dim = 256
        self.cls_extract = nn.Sequential(  # 用于将查询的特征进行融合
            nn.Conv2d(reduce_dim, self.base_classes, kernel_size=1, padding=0, bias=False),
            AReLU(),
        )
    ...
    def forward(self, x, x_cv2, que_name, class_name, y_m=None, y_b=None, s_x=None, s_y=None, cat_idx=None):
        ...
        # triplet loss
        # bq, cq, hq, wq = query_feat_cnn.shape

        class_activation = self.cls_extract(supp_feat_cnn)  # 生成类别激活图 [bs, n_class, h, w]
        # 沿空间维度softmax [bs, n_class, h, w]
        attention_weights = F.softmax(class_activation.view(*class_activation.shape[:2], -1), dim=-1)
        attention_weights = attention_weights.view_as(class_activation)
        expanded_features = supp_feat_cnn.unsqueeze(1)  # 扩展特征图[bs, 1, c, h, w]
        expanded_weights = attention_weights.unsqueeze(2)   # 扩展注意力权重 [bs, n_class, 1, h, w]
        weighted_features = expanded_features * expanded_weights    # 加权特征 [bs, n_class, c, h, w]
        instance_prototype = torch.sum(weighted_features, dim=(3, 4))   # 沿空间维度求和 [bs, n_class, c]
        # instance_prototype = base_supp_prototype.view(bq * self.shot, cq).unsqueeze(1).expand(bq * self.shot,
        # self.base_classes, cq)  # instance prototype
        # instance_prototype = supp_pro.view(bq, cq).unsqueeze(1).expand(bq, 256, cq)

        # obtain local features
        local_features = self.LFG(supp_feat_cnn)
        if self.training:
            # calculate triplet_loss
            triple_loss = get_triple_loss(instance_prototype, local_features, mask, cat_idx)
        else:
        ...
        # # multi
        # img_cam = multi_scale_optimization(img_cam)
        # clip_similarity = multi_scale_optimization(clip_similarity)
        #
        # outputs = process_features(query_feat_cnn, supp_feat_bin, img_cam * 10, clip_similarity * 10)
        # query_feat = self.query_merge(torch.cat(outputs, dim=1))
        ...
        if self.training:
            ...
            return final_out.max(1)[1], main_loss + aux_loss1, distil_loss / 3, aux_loss2, triple_loss
        else:
