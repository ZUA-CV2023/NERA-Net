...
class LRSA(nn.Module):
    def __init__(self, dim, num_heads=2, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 pooled_sizes=None, q_pooled_size=1, q_conv=False):
    ...
    def forward(self, x, H, W, d_convs=None):
        ...
        for (pooled_size, l) in zip(self.pooled_sizes, d_convs):
            ...
            pool = F.adaptive_avg_pool2d(x_, pooled_size).to(l.weight.device)
            pool = pool + l(pool)
            pools.append(pool.view(B, C, -1))

        pools = torch.cat(pools, dim=2)
        pools = self.norm(pools.permute(0, 2, 1).to(next(self.parameters()).device))
...
if __name__ == "__main__":
    ...
    # 初始化输入
    x = torch.randn(B, C, H, W)  # 输入 shape: [B, N, C]
