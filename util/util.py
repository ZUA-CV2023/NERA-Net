...
# 实现多项式学习率衰减策略，可选预热（Warmup）和分层学习率调整（不同参数组设置不同学习率）
def poly_learning_rate(optimizer, base_lr, curr_iter, max_iter, power=0.9, index_split=-1, scale_lr=10., warmup=False,
    ...
    # if curr_iter % 50 == 0: print('Base LR: {:.4f}, Curr LR: {:.4f}, Warmup: {}.'.format(base_lr, lr, (warmup and
    # curr_iter < warmup_step)))
...
# 用于根据输入参数 args 动态生成模型快照和结果的保存路径
...
def convert_to_numpy(img, mean, std):
    img = img.squeeze()
    if len(img.shape) == 3:
...
def save_fig(img, name=None, cmp=None):
    ...
    if isinstance(img, torch.Tensor):
        img = convert_to_numpy(img, mean, std)
